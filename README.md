...
- **Skill-lock timers** — when the game says *"Cannot use the [skill] on this target for another X seconds"* (trimmers!) or *"skill is locked, able in hh:mm:ss"*, a live countdown bar appears above the footer on every tab, drains as the lock runs, and flashes a green **READY** when it's up. Click the locked hotbar button once to (re)start a timer. Prefer a cleaner window? Right-click → *Timers* → **Show cooldown bars** hides them (timers keep tracking in the background, so re-enabling shows current cooldowns)
